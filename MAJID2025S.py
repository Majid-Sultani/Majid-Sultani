...
os.system('xdg-open https://t.me/Majid14232s')
logo = (f"""
\033[35m
 ...
 ❣️ ─────────────────────────────────────────────────🥀
  ...
  \033[1;32m '''ジ气覀 覀气亠亠 
	...
	888b     d888           d8b d8b      888 
	8888b   d8888           Y8P Y8P      888 
	88888b.d88888                        888 
	888Y88888P888  8888b.  8888 888  .d88888 
	888 Y888P 888     "88b "888 888 d88" 888 
...
def Majid():
    clear();print(f'''\033[33m{xd1} \033[33mSTART AUTO CREATE ''');print(f'''{xd0} \033[33mEXIT CLONE ''');linex();mr = input(f'''{xdx}\033[34m SELECT : ''')
    if mr in ('1',):____create____()
    elif mr in ('0',):exit()
    else:linex();print(f'''{xd} OPTION NOT FOUND ''');linex();time.sleep(1);print(f'''{xd} TRY AGAIN ''');time.sleep(1);Majid()
#_________|MODULE|_________#
...
def ____create____():
    ...
    clear();print(f'''{xd} EXAMPLE {R}:{W} Majid {R}|{W} Fahim {R}|{W} Nadia {R}|{W} Asma ''');linex();firstname = input(f'''{xdx} \033[33mENTER FIRST NAME : ''');linex();print(f'''{xd} EXAMPLE {R}:{W} Badakhshi {R}|{W} Afghan {R}|{W} Ahmedi {R}|{W} Ahmadi ''');linex();lastname = input(f'''{xdx} \033[33mENTER LAST NAME : ''');linex();print(f'''{xd} EXAMPLE {R}:{W} 500 {R}|{W} 1000 {R}|{W} 5000{R}|{W} 10000 ''');linex();num_accounts = int(input(f'''{xdx} \033[33mENTER LIMIT : '''));linex();print(f'''{xd} EXAMPLE {R}:{W} 5 {R}|{W} 10{R}|{W} 15 {R}|{W} 20 {R}|{W} 30 {R}|{W} 40 {R}|{W} 50 {R}|{W} 60 ''');linex();delay = int(input(f'''{xdx} \033[33mENTER BETWEEN TIME : '''))
    clear();print(f'''{xd}\033[31m TOTAL CREATE LIMIT {R}:{G} {num_accounts} ''');print(f'''{xd} \033[31mIF NO RESULT ON{R}|{W}\033[1;32mOFF AIRPLANE MODE''');linex()
    for make in range(int(num_accounts)):
        ...
        payload = {
            ...
            'guid': "",
            'pre_form_step': "",
            'encpass': '#PWD_BROWSER:0:{}:{}'.format(str(time.time()).split('.')[0],"Majid@2025"),
            'submit': "Sign Up",
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            print(f'''\r{xd}-{W}[{G}M S -OK{W}]{G} {uid} | Majid@2025 ''')
            print(f'''\r{xd}-{cookie} ''');linex()
            open('/sdcard/MSMAJID-Afghan -CRATE-OK-ID.txt', 'a').write(uid + '|Majid@2025|' + cookie + '\n')
            oks.append(uid)
    except Exception as e:
        pass
